...
The platform: a hoverboard base with an RC car steering assembly, a Raspberry Pi
and an ESP32 on the chassis, and a camera on a telescoping mast. The black tape on
the floor is the guide line.
...
![Electronics on the chassis](docs/media/electronics.jpg)
...
![The mast at full extension](docs/media/mast.jpg)

The mast at full extension, camera at the top. At this height it visibly flexes,
which is what limits accuracy on the upper shelf rows — see `docs/TUNING.md`.

A frame from a calibration sweep:
...
Four IR sensors read the guide line, and correction runs inside every drive
command rather than as a separate mode, so the robot tracks the tape continuously
while covering a commanded distance.

![Line sensors over the guide tape](docs/media/line_sensors.jpg)
