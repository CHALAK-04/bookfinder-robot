/*
============================================================================
  ROBOT DRIVE v7  —  encoder-filter retune + anti-stall approach
============================================================================
  Keeps the V6 anti-stall drive logic, but updates the ESP32 PCNT
  encoder glitch filter to 200 ns to match your newly working encoder test.
  ...
  Also adds:
    - PCNT glitch filter set to 200 ns to match the working measurement sketch
  ...
  COMMANDS:
    ...
    ENCODER: e/E PPCM -/+0.05     z/Z PPCM -/+0.50
...
const int GLITCH_NS = 200;        // MATCHES your fixed measurement sketch; 1000 was missing pulses
...
// ===== CALIBRATION (tunable) =====
float PULSES_PER_CM = 8.60;       // STARTING VALUE ONLY. Retune after GLITCH_NS=200 using count / actual cm
int   kickPower     = 165;        // short start boost
float kickDistCm    = 2.0;        // shorter kick so 10cm is not mostly kick
int   cruisePower   = 150;        // stable retune baseline
int   rampDownCm    = 8;          // smoother approach for retuning
int   crawlPower    = 115;        // final approach power; keep above real minimum moving power
int   targetDistCm  = 30;

// ===== DISTANCE FIX PARAMETERS =====
float stopEarlyCm       = 1.50;   // fixed coast offset; tune AFTER PPCM is correct
float speedCoastPerPWM  = 0.000;  // keep zero while retuning; add later only if needed
...
int   minMovePower      = 110;    // never command below this while still trying to move
...
void printStatus() {
  SerialBT.print("[status] filtNs="); SerialBT.print(GLITCH_NS);
...
void setup() {
  ...
  SerialBT.println("=== ROBOT DRIVE v7 encoder-filter retune + anti-stall ===");
  SerialBT.println("1-9 drive | g set-dist | x stop | ? status | e/E PPCM fine | z/Z PPCM coarse");
  printStatus();
}
