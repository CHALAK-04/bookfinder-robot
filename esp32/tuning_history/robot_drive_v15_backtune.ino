...
void loop() {
  if (SerialBT.available()) {
    ...
    if (c >= '1' && c <= '9') { startDriveDir((c - '0') * 10, pendingDir); pendingDir = +1; }
      ...
      case 'j': backPowerScale -= 0.05; if (backPowerScale < 1.0) backPowerScale = 1.0; printStatus(); break;
      case 'J': backPowerScale += 0.05; if (backPowerScale > 2.0) backPowerScale = 2.0; printStatus(); break;
      case 'l': backCruisePower = max(0, backCruisePower - 5);
                SerialBT.print(">> backCruise="); SerialBT.println(backCruisePower); break;
      case 'L': backCruisePower = min(255, backCruisePower + 5);
                SerialBT.print(">> backCruise="); SerialBT.println(backCruisePower); break;
      case 't': backSteerGain -= 1; if (backSteerGain < 0) backSteerGain = 0;
                SerialBT.print(">> backGain="); SerialBT.println(backSteerGain); break;
      case 'T': backSteerGain += 1;
                SerialBT.print(">> backGain="); SerialBT.println(backSteerGain); break;
      case 'i': backSteerSign = -backSteerSign;
                SerialBT.print(">> backSteerSign="); SerialBT.println(backSteerSign); break;
  ...
  if (driving) {
    ...
    // Direction-aware steering:
    //  - forward: your tuned steerGain, normal sign
    //  - backward: stronger backSteerGain, sign flippable live with 'i'
    int servoCmd;
    if (driveDir < 0) {
      servoCmd = NEUTRAL - backSteerSign * (int)(backSteerGain * filtErr);
    } else {
      servoCmd = NEUTRAL - (int)(steerGain * filtErr);
    }
    writeServo(servoCmd);
